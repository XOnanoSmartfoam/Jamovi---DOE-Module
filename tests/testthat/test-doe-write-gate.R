...
fake_analysis <- function(designOutput) {
    written <- new.env(parent = emptyenv())
    written$touched <- FALSE
    list(
        options = list(designOutput = designOutput, simulateResponses = FALSE),
        results = list(designOutput = list(mark = function() written$touched <- TRUE)),
        written = written
    )
}
...
test_that("the preview tip explains how to send the design", {
    ...
    expect_match(tip, "Send this design to the spreadsheet")
